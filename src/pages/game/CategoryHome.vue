<template>
  <div class="category-home">
    ...
    <div class="banner" :style="bannerStyle">
      ...
      <p class="banner-desc">{{ categoryDesc }}</p>
    </div>
    ...
    <div class="platform-section">
      ...
      <div v-if="games.length" class="platform-grid">
        <div
            v-for="game in games"
            :key="game.sign"
            class="platform-card"
...
import type { ThirdGameItem } from '@/types/common';
...
const categoryTitle = computed(() => {
  ...
  return t(navKeyMap[props.category] ?? '');
});

/** 分类描述（flat key：descSport / descChess / ...） */
const categoryDesc = computed(() => {
  const descKeyMap: Record<string, string> = {
    sport:   'pages.game.descSport',
    chess:   'pages.game.descChess',
    live:    'pages.game.descLive',
    slot:    'pages.game.descSlot',
    fishing: 'pages.game.descFishing',
    e_sport: 'pages.game.descEsport',
  };
  return t(descKeyMap[props.category] ?? '');
});
