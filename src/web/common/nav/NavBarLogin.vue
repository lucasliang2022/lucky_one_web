<template>
  <div class="nav-bar-right is-login">
    ...
    <!-- 站内信:图标 + 右边括号内未读数;无未读不显示数字 -->
    <a class="nav-msg" :title="t('components.nav.message')" @click="goMessages">
      <span class="sd-icon icon-sd-mail nav-msg__icon"></span>
      <span v-if="userStore.unreadMsgCount > 0" class="nav-msg__count">
...
import NavLang from '@web/common/nav/NavLang.vue';
import NavBalance from '@web/common/nav/NavBalance.vue';
import NavTimezone from '@web/common/nav/NavTimezone.vue';
...
const goMessages = () => navToAccount('message');
</script>
...
.nav-msg {
  ...
  &:hover .nav-msg__icon { color: #e4393c; }
}
