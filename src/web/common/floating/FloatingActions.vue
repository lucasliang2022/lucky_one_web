...
import type { FabButtonKey } from '@shared/types/common';
...
const visibleButtons = computed<FabActionDef[]>(() => {
  const enabled = commonStore.tenant?.fab_buttons ?? [];
  return enabled
...
const openSupport = () => {
  const url = commonStore.tenant?.support_url;
  if (url) window.open(url, '_blank');
  else console.warn('tenant.support_url not configured');
};
