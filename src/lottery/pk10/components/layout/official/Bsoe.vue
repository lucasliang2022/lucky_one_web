...
function randomOneBetBsoe(rowsData: MethodRow[], currentMethod: MethodDefineItem): SelectedUnit[][] {
  ...
  if (layoutRows.length > 0) {
    ...
    for (let i = 0; i < layoutRows.length; i++) {
      if (i === randomIndex) {
        ...
        const selectedBalls: SelectedUnit[] = [];
...
function calculateBetCountBsoe(tickets: SelectedUnit[][]): number {
...
const {
  ...
  randomOneBetFn: randomOneBetBsoe,
  calculateCountFn: calculateBetCountBsoe
});
