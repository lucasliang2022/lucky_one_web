<template>
  <div class="ball-show-area tm-bsoe-tail">
...
const checkTailCondition = (codeValue) => {
  const num = parseInt(codeValue);
  if (isNaN(num) || num < 1 || num > 48) {
    return { isTailBig: false, isTailSmall: false, isTailOdd: false, isTailEven: false };
  }
  const tailDigit = num % 10;
  return {
    isTailBig: tailDigit >= 5 && tailDigit <= 9,
    isTailSmall: tailDigit >= 0 && tailDigit <= 4,
    isTailOdd: tailDigit % 2 !== 0,
    isTailEven: tailDigit % 2 === 0
  };
};
...
function calculateSpecificHotCold(range, history, ballsData, positions) {
  ...
  recentIssues.forEach(issue => {
    ...
    if (targetCode) {
      const conditions = checkTailCondition(targetCode);
      if (conditions.isTailBig) results[0]++;
      if (conditions.isTailSmall) results[1]++;
      if (conditions.isTailOdd) results[2]++;
      if (conditions.isTailEven) results[3]++;
    }
  });
  return results;
}
...
function calculateSpecificOmission(history, ballsData, positions) {
  ...
  for (let i = 0; i < history.length; i++) {
    ...
    if (targetCode) {
      ...
      if (conditions.isTailBig && !found[0])   { results[0] = i; found[0] = true; }
      if (conditions.isTailSmall && !found[1]) { results[1] = i; found[1] = true; }
      if (conditions.isTailOdd && !found[2])   { results[2] = i; found[2] = true; }
      if (conditions.isTailEven && !found[3])  { results[3] = i; found[3] = true; }
    }
