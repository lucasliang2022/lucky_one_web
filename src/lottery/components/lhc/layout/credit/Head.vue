...
function calculateHotCold(issueCount, hotColdData) {
  ...
  recentIssues.forEach(issue => {
    const headNum = getHeadNumber(issue);
    if (headNum === null) return;

    if (hotCold[headNum] !== undefined) {
      hotCold[headNum] += 1;
    }
  });
...
function calculateOmission(omissionData) {
  ...
  for (let index = 0; index < issueHistory.value.length; index++) {
    ...
    const headNum = getHeadNumber(issue);
    if (headNum === null) continue;

    if (omission[headNum] === -1) {
      omission[headNum] = index;
    }
