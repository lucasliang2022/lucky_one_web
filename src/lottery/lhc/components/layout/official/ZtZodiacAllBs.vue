...
function calculateHotCold(range, history, ballsData, positions) {
  ...
  recentIssues.forEach(issue => {
    ...
    const isOdd = uniqueZodiacCount % 2 === 1;
    hotCold[isOdd ? 0 : 1]++;
  });
...
function calculateOmission(history, ballsData, positions) {
  ...
  for (let i = 0; i < totalIssues; i++) {
    ...
    if (isOdd && !foundOdd) {
      omission[0] = i;
      foundOdd = true;
    } else if (!isOdd && !foundEven) {
      omission[1] = i;
      foundEven = true;
    }

    if (foundOdd && foundEven) break;
  }

  if (!foundOdd) omission[0] = totalIssues;
  if (!foundEven) omission[1] = totalIssues;
